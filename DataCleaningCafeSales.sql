...
-- Replace item where price is 3.0 with Juice and 4.0 with Sandwich
UPDATE cafe_sales_raw
SET item = 'Juice'
WHERE 
...
UPDATE cafe_sales_raw
SET total_spent = ROUND(CAST(quantity AS NUMERIC) * CAST(price_per_unit AS NUMERIC), 1)::TEXT
WHERE 
  (total_spent IS NULL 
...
WHERE
  ...
  transaction_date = '' OR transaction_date = 'ERROR' OR transaction_date = 'UNKNOWN'; 

-- Just found out 1 error data (No item name, price per unit and total spent))
DELETE FROM cafe_sales_raw
WHERE
	(item = 'UNKNOWN'); -- Just make it simple

-- Change datatype for quantiyt, price per unit, total spent and date
ALTER TABLE cafe_sales_raw
ALTER COLUMN price_per_unit TYPE NUMERIC(10, 1)
...
-- Optional : Remove transaction_id since not relevant
ALTER TABLE cafe_sales_raw
...
-- Check total sale by payment method
SELECT  
payment_method,
